...
const WIDGET_ID = "3c102e3d-7d58-459c-8d09-52fbc37f247e";
...
const Testimonials = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        setIsLoading(true);
        const res = await fetch(`https://featurable.com/api/v2/widgets/${WIDGET_ID}`);
        const data = await res.json();
        
        if (data.success && data.widget?.reviews) {
          const sortedReviews = data.widget.reviews.sort(
  ...
  return (
    <section id="testimonials" className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <CardContent className="p-6 h-full flex flex-col">
                    ...
                    <div className="flex items-start justify-between mt-auto">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="w-12 h-12 bg-primary/10 text-primary rounded-full flex items-center justify-center font-bold text-lg border border-primary/20 group-hover:scale-110 transition-transform duration-300">
                          {getInitial(review.author?.name)}
                        </div>
                        <div>
                          <div className="font-semibold text-foreground text-sm">
                            {review.author?.name || "Verified Patient"}
                          </div>
                          <div className="flex items-center text-xs text-muted-foreground">
                            <MapPin className="w-3 h-3 mr-1" />
                            Google Review
        ...
        {/* CTA Block */}
        <AnimatedElement animation="fadeInUp" delay={300} className="text-center mt-20">
          <div className="bg-primary/5 rounded-3xl p-8 max-w-4xl mx-auto">
            <h3 className="text-2xl font-bold mb-4">Ready to Start Your Healing Journey?</h3>
            <p className="text-muted-foreground mb-6">Book your consultation today and take the first step towards better health.</p>
            <a href="#contact" className="inline-flex items-center px-8 py-3 bg-primary text-primary-foreground rounded-lg font-medium">
              <User className="w-5 h-5 mr-2" />
              Book Consultation
            </a>
          </div>
