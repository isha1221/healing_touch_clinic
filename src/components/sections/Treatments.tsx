...
const Treatments = () => {
  const treatments = [
    {
      ...
      conditions: [ "Teething", "Adenoids", "Behavioral Problems", "Autism","low Immunity", "Asthma in Children"],
      color: "primary",
    ...
    },
    {
      ...
      conditions: [ "Piles", "Acidity", "Constipation", "Acid reflux", "Anal fissure","IBS"],
      color: "accent",
    ...
    },
    {
      ...
      conditions: ["Hypertension", "Palpitations", "High Cholesterol", "Cardiac Anxiety", "Heart Faliure"],
      color: "primary",
      gradient: "bg-primary/10"
    },
    {
      ...
      conditions: ["Anxiety", "Depression", "OCD", "Insomnia", "Panic Attacks", "Eating Disorders"],
      color: "secondary",
      gradient: "bg-secondary/10"
    },
    {
    ...
    },
    {
      ...
      conditions: ["Arthritis", "Gout", "Osteoporosis", "Cervical Spondylosis", "Sciatica", "Sports Injuries"],
      color: "primary",
      gradient: "bg-primary/10"
    },
    {
      ...
      conditions: ["Eczema", "Psoriasis", "Acne", "Urticaria", "Corn &Warts", "Alopecia"],
      color: "accent",
      gradient: "bg-accent/10"
    },
    {
  ...
  const getShadowClasses = (index: number) => {
    const shadowIndex = index % 3;
    switch (shadowIndex) {
      case 0:
        return 'hover:shadow-healing';
      case 1:
        return 'hover:shadow-warm';
      case 2:
        return 'hover:shadow-hot';
      default:
        return 'hover:shadow-healing';
    }
  };

  const getBadgeClasses = (cardIndex: number) => {
    const shadowIndex = cardIndex % 3;
    switch (shadowIndex) {
      case 0:
        return 'border-primary hover:bg-primary/10 hover:text-primary';
      case 1:
        return 'border-secondary hover:bg-secondary/10 hover:text-secondary';
      case 2:
        return 'border-red hover:bg-red/10 hover:text-red';
      default:
        return 'border-primary hover:bg-primary/10 hover:text-primary';
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-16">
          ...
          <div className="text-center">
            ...
            <h3 className="font-semibold text-foreground mb-2">Physical and Mental Well being</h3>
            <p className="text-sm text-muted-foreground">Stress managment</p>
          </div>
          ...
          <div className="text-center">
            ...
            <h3 className="font-semibold text-foreground mb-2">Suitable for all Ages</h3>
  ...
  // Check if the icon is a string (image path) or a Lucide component
  const isCustomImage = typeof Icon === 'string';

  return (
    <Card 
      key={index} 
      className={`group ${getShadowClasses(index)} transition-all duration-300 border-border hover:border-primary/30`}
    >
      <CardHeader className="pb-4">
        <div className={`w-16 h-16 ${treatment.gradient} rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
          {isCustomImage ? (
            ...
            <Icon className={`w-8 h-8 ${getIconColor(treatment.color)}`} />
          )}
        </div>
        {/* ... rest of your CardHeader and CardContent */}
                </CardHeader>
                
                <CardContent>
                  <div className="space-y-3">
                     <CardTitle className="text-xl text-foreground  transition-colors">
                    {treatment.title}
                  </CardTitle>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {treatment.conditions.map((condition, conditionIndex) => (
                        <Badge 
                          key={conditionIndex} 
                          variant="secondary" 
                          className={`text-xs flex items-center text-foreground bg-transparent ${getBadgeClasses(index)} transition-colors`}
                        >
                          {condition}
                        </Badge>
                      ))}
